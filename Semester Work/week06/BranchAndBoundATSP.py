...
if ScenarieNr == 1:
    ...
    # Add that we can go from any ending activity to any start activity for free
    n = 6
    c = [
            [M, 4, 9,13, 7, 0],
            [14, M,12, 8, 5, 0],
    ...
    # Add dummy such that we can go from E to A for free, thus we will always pick this route when minimizing.
    n = 6
    c = [
            [ M, 4, 9,13, 7, M],
            [14, M,12, 8, 5, M],
...
def branch_and_bound_atsp(cost, included_edges, excluded_edges, lower_bound,
    ...
    This implements the little et al. algorithm from uge 10, where we compute solutions
    based on row/columns reductions. The tree is printed and the feasible solution with
    the lowest bound is the optimal solution. Note as solution like [(2, 0), (3, 2), (0, 4), (1, 3)]
    corresponds to the tour 0 -> 4 -> 1 -> 3 -> 2 -> 0. This can be deduced from the fact that
    node 4 must have 1 inflow and outflow and the same for node 1. DRAW THIS ON PAPER
